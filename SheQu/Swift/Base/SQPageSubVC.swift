...
class SQPageSubVC: UIViewController {
    var accountID = SQAccountModel.getAccountModel().account_id
    var tableHeaderViewHeight: CGFloat = 30
    var canScroll = false
    var isFatherView = false
    ...
    lazy var tableView: SQControlEventTableView = {
        ...
        let tableView        = SQControlEventTableView.init(frame: frame, style: style)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if !self.isKind(of: vc.classForCoder)
            ...
            && !self.isKind(of: SQFriendVC.self) {
            if isFirstDisp {
                isFirstDisp = false
                addNavBtn()
            }else{
...
extension SQPageSubVC: UIGestureRecognizerDelegate {
...
extension SQPageSubVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if (scrollView.contentOffset.y <= 0) {
             canScroll = false
            scrollView.contentOffset = CGPoint.zero
            NotificationCenter.default.post(name: SQControlEventTableView.noti, object: nil)
        }
