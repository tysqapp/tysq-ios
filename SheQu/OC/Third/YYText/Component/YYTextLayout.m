...
static void YYTextDrawBorderRects(CGContextRef context, CGSize size, YYTextBorder *border, NSArray *rects, BOOL isVertical) {
    ...
    NSMutableArray *leftPaths = [NSMutableArray new];
    for (NSValue *value in rects) {
        ...
        if(border.leftLineWidth != 0) {
            //CGContextSetLineWidth(r, <#CGFloat width#>)
            UIBezierPath *leftPath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(rect.origin.x, rect.origin.y, border.leftLineWidth, rect.size.height) cornerRadius:border.cornerRadius];
            [leftPath closePath];
            [leftPaths addObject:leftPath];
        }
        [path closePath];
    ...
    if (border.fillColor) {
        ...
        if (leftPaths.count > 0) {
            CGContextSaveGState(context);
            CGContextSetFillColorWithColor(context, border.leftLineLColor.CGColor);
            for (UIBezierPath *path in leftPaths) {
                CGContextAddPath(context, path.CGPath);
            }
            CGContextFillPath(context);
            CGContextRestoreGState(context);
