...
+ (instancetype)borderWithFillColor:(UIColor *)color cornerRadius:(CGFloat)cornerRadius {
    ...
    one.insets = UIEdgeInsetsMake(-10, 0, -10, 0);
    return one;
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    [aCoder encodeObject:self.leftLineLColor forKey:@"leftLineLColor"];
    [aCoder encodeObject:@(self.leftLineWidth) forKey:@"leftLineWidth"];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    _leftLineWidth = ((NSNumber *)[aDecoder decodeObjectForKey:@"leftLineWidth"]).floatValue;
    _leftLineLColor = [aDecoder decodeObjectForKey:@"leftLineLColor"];
    return self;
}
